defmodule AgentbotCore.Repo.Migrations.CreateCapabilities do
  ...
  def change do
    create table(:capabilities) do
      add :name, :string, null: false              # ör: "code.review", "sap.fi.reconciliation"
      add :description, :string
      add :category, :string                        # code, sap, research, data, devops...
      add :status, :string, default: "active"       # active, deprecated
      add :metadata, :text                           # JSON — verification criteria, etc.
    ...
    create table(:agent_capabilities, primary_key: false) do
      ...
      add :verified, :boolean, default: false        # human doğrulamış mı?
    ...
    create table(:capability_gaps) do
      add :capability_name, :string, null: false     # "sap.einvoice.validation"
      add :requested_count, :integer, default: 1     # kaç kez talep edildi
      add :last_requested_at, :utc_datetime
      add :fulfilled, :boolean, default: false       # bir agent kaydedince true olur
      add :fulfilled_by, :string                      # agent_id
